...
export const REDUCED_TEST_INCLUDE = [
  '**/__tests__/helpers/**/*.test.{ts,tsx}',
  '**/__tests__/types/**/*.test.{ts,tsx}',
  '**/__tests__/lib/api.base-url.test.ts',
  '**/__tests__/lib/api.upload-image-reference.test.ts',
  '**/__tests__/lib/api.product-image-payload.test.ts',
  ...
  '**/__tests__/lib/spa-redirect.test.ts',
  '**/__tests__/components/MarkdownText.test.tsx',
...
export function createVitestConfig(mode: string, include?: string[]) {
  ...
  return {
    ...
    test: {
      ...
      include: include || ['**/__tests__/**/*.test.{ts,tsx}'],
      exclude: [
        '**/node_modules/**',
        '**/dist/**',
        '**/cypress/**',
